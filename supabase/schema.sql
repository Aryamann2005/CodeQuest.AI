-- Clean installation schema for CodeQuest.AI.
...
create table if not exists public.profiles (
  ...
  title text not null default 'Code Apprentice',
...
begin
  ...
  values (
    new.id,
    coalesce(new.raw_user_meta_data ->> 'full_name', 'New Hero'),
...
create table if not exists public.problem_completions (
  ...
  problem_id text not null references public.challenges(id),
